...
**Note**: `validateSanityQuery` only validates query strings, not parameters. If you define queries that use dynamic parameters and rely on `validateSanityQuery`, you must ensure those queries are safe to run with any parameter value.
