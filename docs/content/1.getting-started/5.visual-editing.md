...
Content Variants is a closed Studio beta (`beta.variants.enabled`) and is not publicly documented yet. When Presentation has a variant selected, this module persists it and sends it with `useSanityQuery`. Read the current variant with `useSanityPreviewVariant()`.
...
Until Content Variants leaves beta, the Sanity client must use the `vX` API alias (`apiVersion: 'X'`). This does not change the module default:
