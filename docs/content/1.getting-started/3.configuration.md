...
The array exported from `schemaTypesPath` remains the source of the workspace
schema. When a Sanity config file exists at `configFile`, type generation also
resolves plugins from that config so their contributed schema types are
included.
